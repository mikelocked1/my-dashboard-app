...
const SimpleHealthAnalytics: React.FC = () => {
  ...
  const { data: healthData, isLoading: healthLoading } = useQuery<HealthData[]>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/health-data/${userProfile.id}?${params.toString()}`);
      if (!response.ok) {
  ...
  const { data: healthAlerts, isLoading: alertsLoading } = useQuery<HealthAlert[]>({
    ...
    queryFn: async () => {
      if (!userProfile?.id) return [];
      const response = await fetch(`/api/health-alerts/${userProfile.id}?limit=10`);
      if (!response.ok) {
  ...
  const getMetricStats = (type: string) => {
    ...
    const typeData = healthData.filter((item: HealthData) => item.type === type);
    if (typeData.length === 0) return null;
    
    const values = typeData.map((item: HealthData) => {
  ...
  return (
    <div className="space-y-6" data-testid="health-analytics">
      ...
      {/* Health Alerts */}
      {healthAlerts && Array.isArray(healthAlerts) && healthAlerts.length > 0 && (
        <Card data-testid="health-alerts-section">
          ...
          <CardContent>
            <div className="space-y-3">
              {healthAlerts.slice(0, 3).map((alert: HealthAlert) => (
                <div 
                  key={alert.id}
      ...
      {/* Detailed Data Table */}
      {showDetails && healthData && Array.isArray(healthData) && healthData.length > 0 && (
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {healthData.slice(0, 20).map((item: HealthData) => (
...
export default SimpleHealthAnalytics;